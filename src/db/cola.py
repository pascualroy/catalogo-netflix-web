"""
cola.py — Repositorio de netflix_cola: lectura, marcado de estados y backoff.
"""
...
def siguiente_url(cur):
    ...
    # Fase 1
    cur.execute("""
        SELECT url, id_plataforma FROM netflix_cola
...
def marcar_en_proceso(cur, url: str):
    cur.execute("UPDATE netflix_cola SET estado='en_proceso' WHERE url=?", (url,))
...
def marcar_error(cur, url: str, mensaje: str, http_status=None):
    ...
    cur.execute("SELECT intentos FROM netflix_cola WHERE url=?", (url,))
    fila = cur.fetchone()
...
def añadir_url_cola(cur, url: str, fuente: str = "recomendacion", id_titulo_origen=None):
    id_plataforma = extraer_id_netflix(url)
    try:
        cur.execute("""
            INSERT IGNORE INTO netflix_cola (url, id_plataforma, fuente, id_titulo_origen)
            VALUES (?, ?, ?, ?)
